...
import logging
import json
from deepgpt.pt.deepgpt_constants import *

TENSOR_CORE_ALIGN_SIZE = 8
ADAM_OPTIMIZER = 'adam'
LAMB_OPTIMIZER = 'lamb'
DEEPGPT_OPTIMIZERS = [ADAM_OPTIMIZER, LAMB_OPTIMIZER]
...
class DeepGptConfig(object):
    def __init__(self, json_file, mpu=None):
        super(DeepGptConfig, self).__init__()
    ...
    def _initialize_params(self, param_dict):
        ...
        if self.optimizer_name is not None and \
            self.optimizer_name.lower() in DEEPGPT_OPTIMIZERS:
    ...
    def _do_batch_size_sanity_check(self):
        ...
        if self.train_micro_batch_size_per_gpu is not None:
            assert self.gradient_accumulation_steps is None, \
                'DeepGptConfig: {} and {} should not be defined together' \
            ...
            assert self.train_batch_size % self.train_micro_batch_size_per_gpu == 0, \
                'DeepGptConfig: {} {} is not divisible by {} {}' \
            ...
            if per_device_batch_size > self.train_micro_batch_size_per_gpu:
                assert per_device_batch_size % self.train_micro_batch_size_per_gpu == 0, \
                    'DeepGptConfig: Per device batch size {} is not divisible by {} {}' \
    ...
    def _do_error_check(self):
        if self.zero_enabled:
            assert self.fp16_enabled, "DeepGptConfig: ZeRO is only supported if fp16 is enabled"

        assert self.train_micro_batch_size_per_gpu, "DeepGptConfig: {} is not defined".format(TRAIN_MICRO_BATCH_SIZE_PER_GPU)

        assert self.gradient_accumulation_steps, 'DeepGptConfig: {} is not defined'.format(
            GRADIENT_ACCUMULATION_STEPS)
    ...
    def _do_warning_check(self):
        ...
        if self.gradient_clipping > 0. and not fp16_enabled:
            logging.warning(
                'DeepGptConfig: gradient clipping enabled without FP16 enabled.')

        vocabulary_size = self._param_dict.get(VOCABULARY_SIZE, VOCABULARY_SIZE_DEFAULT)
        if vocabulary_size and vocabulary_size % TENSOR_CORE_ALIGN_SIZE != 0:
            logging.warning(
                "DeepGptConfig: vocabulary size {} is not aligned to {}, may import tensor core utilization."
        ...
        if self.optimizer_params is not None and \
            ...
            if fp16_enabled:
                logging.warning(
                    'DeepGptConfig: In FP16 mode, DeepGpt will pass {}:{} to FP16 wrapper'
                    .format(MAX_GRAD_NORM,
                            self.optimizer_params[MAX_GRAD_NORM]))
            else:
                logging.warning(
                    'DeepGptConfig: In FP32 mode, DeepGpt does not permit MAX_GRAD_NORM ({}) > 0, setting to zero'
